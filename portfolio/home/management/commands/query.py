...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: CommandParser) -> None:
        ...
        parser.add_argument("-k", "--top_k", default=settings.TOP_K, type=int, help="Amount of documents to return (Default: 4)")
        parser.add_argument("-fk", "--fetch_k", default=settings.FETCH_K, type=int, help="Amount of documents to pass to MMR algorithm (Default: 20)")
        parser.add_argument("-l", "--lambda_multiplier", default=settings.LAMBDA_MULTIPLIER, type=float, help="Lambda multiplier for MMR algorithm (Default: 0.5 | 0.0 = only similarity, 1.0 = only diversity)")
        ...
        parser.add_argument("-s", "--search_type", default=settings.SEARCH_TYPE, type=str, help="Search type to use - Options: mmr, similarity_score_threshold")
    ...
    def handle(self, *args: Any, **options: Any) -> str | None:
        try:
            ...
            embedding_function = OpenAIEmbeddings(model=options["embedding_type"])
            # Load Chroma DB
            db = Chroma(
                persist_directory=options["input_directory"], embedding_function=embedding_function)
            # Retriever for Chroma DB
            search_kwargs = {}
